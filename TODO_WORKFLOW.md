...
<!-- content -->
**Context.** On 2026-05-18 the V2 §2.3 prose was ported from [analytics/docs/section_2_3.tex](analytics/docs/section_2_3.tex) into [analytics/docs/main_v2.tex](analytics/docs/main_v2.tex) §2.3 (line 482). The PDF compiles cleanly (22 pages, ~2.9 MB). Companion [analytics/docs/Appendix.tex](analytics/docs/Appendix.tex) compiles standalone (7 pages, ~1 MB). See [LP_TEX_REF.md](LP_TEX_REF.md) at the repo root for the LaTeX conventions reference, figure mapping tables, and compile commands. See the 2026-05-18 entry in [WORKLOG.md](WORKLOG.md) for the full session record.
...
2. **Address remaining flagged items in [LP_TEX_REF.md](LP_TEX_REF.md):**
   ...
   - The `analytics/docs/section_2_3.tex` standalone fragment — delete if you no longer want it as a reference copy, since its content is now in `main_v2.tex`.
3. **Port `main_v2.tex` → `main.tex`** (agent task, mechanical):
   ```bash
   rm "analytics/docs/main.tex"
   mv "analytics/docs/main_v2.tex" "analytics/docs/main.tex"
   cd analytics/docs/ && latexmk -pdf main.tex
   rm analytics/docs/main_v2.pdf  # if not already gone
   ```
