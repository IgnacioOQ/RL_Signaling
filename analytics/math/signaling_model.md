...
i.i.d. across episodes. Equivalently, each component $v_{t,k}$ is an independent fair coin (Phase 1 Axis 1).
...
This is the entry `agents_observed_variables[i]` in the env constructor. The lists are not required to be disjoint — overlap between agents is allowed (Phase 1 Axis 2).
...
Fix a finite directed graph $G = (V, E)$ on the agent set $V = \{0, 1, \dots, N-1\}$. The convention (Phase 1 Axis 7) is
...
**Self-loops** $(i, i) \in E$ are permitted by the env and not filtered, but assumed absent in practice (Phase 1 Axis 8). **Parallel edges** are undefined behaviour: the env requires `nx.DiGraph` (single edge per ordered pair), not `nx.MultiDiGraph` (Phase 1 Axis 9).
...
The state key is the **full** $\mathbf{v}$, regardless of `full_information` (Phase 1 Axis 10). This is what makes signaling necessary in the partial-information regime: an agent's payoff depends on features it cannot directly observe, so coordination via signals is required to reach the optimum.
...
The signal step is **simultaneous** (Phase 1 Axis 6): every agent samples from $\pi_i^{\text{sig}}(\cdot \mid \mathbf{o}_i)$ before any signal is delivered, so no agent's signal can depend on another agent's signal in the same episode.
...
When `costly_signaling=True`, signals equal to the null index $K$ are **suppressed** during propagation (Phase 1 Axis 5): they are not appended to the receiver's observation. So if all of $i$'s in-neighbours emit null, $\tilde{\mathbf{o}}_i = \mathbf{o}_i$ (length unchanged); if some emit null and others don't, only the non-null ones are appended (variable length, dependent on the realization).
...
The Phase 1 spec (Axis 16) distinguishes three primary regimes by the boolean flags `(full_information, with_signals)`:
