...
class UrnAgent:
    def __init__(self, n_signaling_actions, n_final_actions,
                 n_observed_features=1, initialize_urns=False):
        ...
        if not initialize_urns:
            self.signalling_urns = {}  # Dictionary for signaling action distributions
            self.action_urns = {}  # Dictionary for final action distributions
        else:
            # Assumes create_initial_signals is defined elsewhere and initializes urns
    ...
    def get_action(self, state, is_signaling=True):
        ...
        if is_signaling:
            if state not in self.signalling_urns:
                self.signalling_urns[state] = np.ones(n_actions)  # Start with equal probability
        else:
            if state not in self.action_urns:
                self.action_urns[state] = np.ones(n_actions)  # Start with equal probability
